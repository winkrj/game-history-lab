...
if [[ ! -f "$result_dir/metadata.txt" ]]; then
    echo "Not a Stage 1 benchmark result directory: $result_dir" >&2
    exit 1
fi
...
capture_case() {
    ...
    local query_sql
    query_sql="$(build_query "$from_value" "$offset_value")"

    printf '%s\n' "$query_sql" > "$explain_dir/$case_id.sql"
    "${mysql_client[@]}" --batch --raw --skip-column-names \
        --execute "EXPLAIN ANALYZE FORMAT=TREE $query_sql" \
        > "$explain_dir/$case_id-analyze-tree.txt"
    "${mysql_client[@]}" --batch --raw --skip-column-names \
        --execute "EXPLAIN FORMAT=JSON $query_sql" \
        > "$explain_dir/$case_id-estimate.json"
    "${mysql_client[@]}" --table \
        --execute "EXPLAIN FORMAT=TRADITIONAL $query_sql" \
        > "$explain_dir/$case_id-traditional.txt"
}
